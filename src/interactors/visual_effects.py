...
class VisualEffects:
    ...
    def _spawn_celebration_particles(self, player: int) -> None:
        ...
        for _ in range(20):
            ...
            colors = [self.COLOR_NEON_PINK, self.COLOR_NEON_BLUE, 
                     self.COLOR_NEON_PURPLE, self.COLOR_NEON_CYAN]
    ...
    def _spawn_impact_particles(self, position: tuple) -> None:
        ...
        for _ in range(12):
            ...
            # Random neon color
            colors = [self.COLOR_NEON_CYAN, self.COLOR_NEON_YELLOW, 
                     self.COLOR_NEON_ORANGE, self.COLOR_WHITE]
            particle.GetProperty().SetColor(*random.choice(colors))
    ...
    def toggle_trail(self) -> bool:
        ...
        if not self.trail_enabled:
            self.clear_trail()
        logger.info(f"Neon trail effect {'enabled' if self.trail_enabled else 'disabled'}")
